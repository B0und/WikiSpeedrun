...
testWithMSW("updates and retains the live article presentation", async () => {
  ...
  await expect
    .poll(() =>
      document.querySelector("[data-wiki-article-width]")?.getAttribute("data-wiki-article-width"),
    )
    .toBe("standard");
  ...
  await expect
    .poll(() => (articleBody ? getComputedStyle(articleBody).fontSize : null))
    .toBe("20px");
  ...
  await expect
    .poll(() => (articleColumn ? getComputedStyle(articleColumn).maxWidth : null))
    .toBe("none");
  ...
  await expect.element(screen.getByRole("radio", { name: "Wide" })).toBeChecked();
