  ...
  return (
    <>
      <div className="border-secondary-border border-b-[1px]">
        <InfoTooltip>
          <h2 className="font-serif text-3xl">{t("Statistics")}</h2>
        </InfoTooltip>
      </div>
      <div className="@container">
        <ul className="flex @3xl:max-w-[calc(min(50%,700px))] flex-col gap-3 pt-4">
          <Stat name={t("Wins:")} value={wins} />
          <Stat name={t("Total games:")} value={totalRuns} />
          ...
          <Stat name={t("Known languages:")} value={knownWikiLanguages.length} />
          <Stat name={t("Random choices:")} value={random1Pressed + random5Pressed} />
          <Stat name={t("Articles clicked:")} value={articleClicks} />
          <Stat name={t("Previewed Articles:")} value={articlePreviewPressed} />
        </ul>
      </div>
    </>
  );
